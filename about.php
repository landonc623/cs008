<?php include '../top.php'; ?>

<main>
    <article>
        <section class="flexslider">
            <ul class="slides">
                <li>
                    <img src="../images/slider/jquery1.png" alt="" class="rotating-item">
                </li>
                <li>
                    <img src="../images/slider/jquery2.jpeg" alt="" class="rotating-item">
                </li>

                <li>
                    <img src="../images/slider/jquery3.jpeg" alt="" class="rotating-item">
                </li>
            </ul>
        </section>
        
        <p>Welcome to Picture Studios! We believe that every photo tells a story, and because of that, it is our mission to provide the upmost quality of photos for every occasion. 
           Founded in November of 1990, we have over 25 years in the photography industry, and our talented photographers really display our years if experience! Located in Burlington
           Vermont, we are a local small business with around 10 employees, ranging from landscape to wedding photography. We can do small and large events, and also videography for a 
           variety of gatherings. Our company also does portraits as well.</p>

        <h3>How we Were Founded:</h3>
        <p>As said above, we were founded 28 years ago in November of 1990. We initially started with just one photographer, and grew to now have fifteen photographers employed! 
        Our founder, Tina Belcher, enjoys photographing landscape images from the beautiful outdoors in Vermont. Tina at her local high school would take pictures at all of the sporting
        events, and even took some Senior photos of her fellow classmates. Originally from Ocean City, New Jersey, Tina decided to move to Vermont and attend UVM, graduating 
        in 1988 with her degree in Photography from the Department of Art and Art History. Once she realized she loved Burlington too much to leave, she decided to start up her
        own small business, Picture Studios, in Burlington Vermont.</p>

        <h3>What Photographing We Do:</h3>
        <p>All our photographers are specialized in one event, but have the skill and talent to do any event assigned to them. We can do a multitude of different occasions, 
            ranging from:</p>

        <ul id="events">
            <li>Landscape Images</li>
            <li>Travel Photography</li>
            <li>Outdoor Sports</li>
            <li>Weddings</li>
            <li>Birthdays</li>
            <li>Concerts</li>
            <li>Portraits</li>
            <li>Corporate Events</li>
        </ul>

        <h3>Our Photography Style:</h3>
        <p>After over 25 years in the industry, we have perfected our style and quality of our images. At all of our events, we remain outside of the occasion, saving the
        spotlight for our customers and capturing the day as it goes. In portraits, we will take our time with you, allowing you to see the images as we take them and 
        decide on what you like and dislike. We also shoot in a variety of locations for portraits, from our studio which is decked out with decorations including a 
        Christmas tree and a fake living room, to your own home or outside.</p>

        <h3>How Many Photos Should I Expect to Receive:</h3>
        <p>Your should expect receive around 100-200 photos for larger events, including corporate events and weddings. For portrait style, we have different packages with
        differing amount and types of photos. On average we take around 1,000-2,000 photos at large events, but after editing out duplicates, odd expressions, and images
        that do not meet our standards for lighting and quality, we end up with about 100 fully finished images.</p>

        <h3>When Should I Expect To See My Photos:</h3>
        <p>Depending on the size of the event and the amount of photos taken, you should expect to see your photos done 6 to 8 weeks after the event. For portraits, you
        should expect to see them 2-3 weeks after being taken, sometimes longer depending on if it’s the holiday season.</p>
    </article>
</main>

<?php include '../footer.php' ?>

</body>
</html>